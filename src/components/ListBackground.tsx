...
const ListBackground: React.FC<ListComponentProps> = ({
  ...
  const selectForm = (id: string) => {
    switch (id) {
      case '01': 
      case '02':
        return <FormLayoutOutreach read={true} data={data}></FormLayoutOutreach>;
      case '03' :
      ...
      case '12' :
        return <FormLayoutExercise read={true} data={data}></FormLayoutExercise>
      case '13' :
  ...
  const onCheck = async (id: number, e: React.MouseEvent<HTMLDivElement>) => {
    await axios.post('http://www.zp11489.tld.122.155.167.85.no-domain.name/www/update-act.php', JSON.stringify({act_id: id, flag: 3}))
    .then((res) => {
        console.log(res)
  ...
  const onWrong = async (id: number, e: React.MouseEvent<HTMLDivElement>) => {
    await axios.post('http://www.zp11489.tld.122.155.167.85.no-domain.name/www/update-act.php', JSON.stringify({act_id: id, flag: 4}))
    .then((res) => {
        console.log(res)
        if (refresh) refresh(e)
        setOpen(false)
  ...
  return (
    <div className="mb-2 w-full " onClick={navigate}>
      <div className={`grid grid-cols-6 bg-pccp-light-orange p-3 pb-4 gap-x-3 ${open ? "rounded-t-lg" : "rounded-lg"}  flex items-stretch`}>
        ...
        <div className="text-right self-center mt-2 mr-2" onClick={() => setOpen(!open)}>
          <IonIcon src={
            !teacher ? chevronForward : open ? caretUp : caretDown 
            }></IonIcon>
        </div>
      </div>
      <div className={`bg-pccp-light-blue rounded-b-lg ${open ? "max-h-screen" : "hidden"}`}>
            {selectForm(act)}
            <div className="px-5 pb-7 pt-0 grid grid-cols-2 gap-x-3 w-full text-center content-center text-white">
                <div className="bg-danger rounded-md p-1.5 shadow" onClick={(e) => {if (data) onWrong(data.act_id, e)}}>ส่งคืน</div>
                <div className="bg-success rounded-md p-1.5 shadow" onClick={(e) => {if (data) onCheck(data.act_id, e)}}>ตรวจ</div>
            </div>
