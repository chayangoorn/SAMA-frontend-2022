...
const SearchPage: React.FC = () => {
    ...
    const getName = async () => {
        await axios.post('http://www.zp11489.tld.122.155.167.85.no-domain.name/www/selectname.php',
    ...
    const getClassPoints = async () => {
      if (!loading) {
        setLoading(true)
        await axios.post("http://www.zp11489.tld.122.155.167.85.no-domain.name/www/achieve.php", JSON.stringify({classroom: room}))
        .then((res) => {
          setLoading(false)
        })
      }
